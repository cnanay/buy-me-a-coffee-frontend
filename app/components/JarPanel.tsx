...
import { TxToast, type TxStatus } from './TxToast';
...
type Props = {
  /** The tip jar contract to talk to (V1 or a factory-created V2 jar —
   *  both share the same tipping ABI). */
  address: `0x${string}`;
  /** Called with the supporter count whenever memos load/refresh. */
  onSupporters?: (count: number, loading: boolean) => void;
};

/** The full tipping experience for one jar: tip form, owner withdraw,
 *  supporter wall, tx toast + confetti. */
export function JarPanel({ address: jarAddress, onSupporters }: Props) {
  ...
  const { data: tipsData, refetch: refetchTips } = useQuery<TipsResponse>({
    queryKey: ['tips', jarAddress],
    queryFn: async () => {
      const res = await fetch(`/api/tips?address=${jarAddress}`);
  ...
  // Let the parent (e.g. a stats bar) mirror the supporter count.
  useEffect(() => {
    onSupporters?.(memoList.length, memosLoading);
  }, [memoList.length, memosLoading, onSupporters]);
